...
pub fn LoginPage() -> impl IntoView {
    ...
    let handle_login = {
        let navigate = navigate.clone();
        move |_| {
            let identifier_val = identifier.get();
            let password_val = password.get();

            if identifier_val.is_empty() || password_val.is_empty() {
                set_error.set("Please enter your username/email and password".to_string());
                return;
            }
            ...
            let navigate = navigate.clone();
            spawn_local(async move {
                match login(identifier_val, password_val).await {
                    Ok(response) => {
                        if response.success {
                            set_is_loading.set(false);
